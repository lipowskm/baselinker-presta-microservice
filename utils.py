...
def get_product_buy_price(
    presta_url: str, presta_token: str, product_id: str, multiplier: float
) -> Optional[float]:
    if not product_id or product_id == "0":
        return 0.0
    endpoint = f"api/products/{product_id}"
    url = f"{presta_url.rstrip('/')}/{endpoint}?ws_key={presta_token}"
...
def get_products(
    baselinker_url: str, baselinker_token: str, order_id: str
) -> Union[str, dict]:
    ...
    response = requests.request("POST", baselinker_url, headers=headers, data=payload)
    if not response.ok:
