...
from utils import (BaseLinkerApiError, PrestaError, get_product_buy_price,
                   get_products, update_price)
...
def hello_world(order_id: str):
    try:
        products_dict = get_products(BASELINKER_TOKEN, order_id)
    ...
    for order_product_id, product_id in products_dict.items():
        try:
            new_price = get_product_buy_price(PRESTA_TOKEN, product_id, MULTIPLIER)
    ...
    for order_product_id, new_price in prices_to_update.items():
        try:
            update_price(BASELINKER_TOKEN, order_id, order_product_id, new_price)
        except PrestaError as e:
            return Response(
